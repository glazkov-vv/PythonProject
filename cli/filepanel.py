...
class FilePanel(urwid.Filler):
    ...
    def __init__(self,custom_data,workspace:Workspace,pos:int=0) -> None:
        ...
        lbx=urwid.ListBox([urwid.Text("OPS")]+[TitleEntry(self._custom_data)]+[FileEntry(self._custom_data,h,self.pos,workspace) for h in workspace.get_contents()])
    ...
    def rebuild(self,in_focus:bool=False)->None:
        lbx=urwid.ListBox([urwid.Text("OPS")]+[TitleEntry(self._custom_data)]+[FileEntry(self._custom_data,h,self.pos,self._workspace) for h in self._workspace.get_contents()])
